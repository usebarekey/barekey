...
type RevealedVariableValue =
	...
	| { kind: "ab_roll"; declaredType: DeclaredVariableType; valueA: string; valueB: string; chance: number }
	| {
			kind: "rollout";
			declaredType: DeclaredVariableType;
			valueA: string;
...
type DialogMode =
	| { mode: "closed" }
	| { mode: "create" }
	| { mode: "edit-existing"; row: Extract<VariableDisplayRow, { type: "existing" }>; loading: boolean }
...
function normalizeDraftValue(declaredType: DeclaredVariableType, raw: string): string {
	...
	if (declaredType === "int64") {
		const t = raw.trim();
		if (!/^-?(0|[1-9]\d*)$/.test(t)) throw new Error("Integer variables must be valid signed 64-bit integers.");
	...
	if (declaredType === "float") {
		const t = raw.trim();
		if (t.length === 0 || !Number.isFinite(Number(t))) throw new Error("Float variables must be finite numbers.");
		return t;
	}
	if (declaredType === "date") {
		const t = raw.trim();
		if (!RFC3339_WITH_TIMEZONE_PATTERN.test(t)) throw new Error("Date variables must be ISO 8601 with timezone.");
		const parsed = new Date(t);
		if (Number.isNaN(parsed.getTime())) throw new Error("Date variables must be ISO 8601 with timezone.");
...
function createEmptyDraftState(): StageDraftState {
	return { newRows: [], updatedValues: {}, deletedIds: {}, revealedValues: {}, revealedIds: {}, decryptingIds: {} };
}

function hasPersistedDraftChanges(draft: StageDraftState): boolean {
	return draft.newRows.length > 0 || Object.keys(draft.updatedValues).length > 0 || Object.keys(draft.deletedIds).length > 0;
}
...
function createDefaultRolloutMilestone(percentage = 0, at = new Date().toISOString()): RolloutMilestone {
	return {
		at,
		percentage,
	};
...
function createLoadingDialogForm(row: Extract<VariableDisplayRow, { type: "existing" }>): DialogForm {
	if (row.kind === "secret") {
		return {
			name: row.name,
			kind: "secret",
								...
								<div className="grid grid-cols-2 gap-3">
									<div className="space-y-1.5">
										<Label htmlFor="var-value-a">
											Value A <span className="text-muted-foreground font-normal">({Math.round(chance * 100)}%)</span>
										</Label>
										{isJson ? (
											<CodeEditor
												id="var-value-a"
									...
									<div className="space-y-1.5">
										<Label htmlFor="var-value-b">
											Value B <span className="text-muted-foreground font-normal">({Math.round((1 - chance) * 100)}%)</span>
										</Label>
										{isJson ? (
											<CodeEditor
												id="var-value-b"
...
function VariableRow({
	...
	const rolloutFunction =
		row.kind === "rollout" ? (row.type === "existing" ? row.rolloutFunction : row.rolloutFunction) : null;
		...
		>
			...
			<div className="min-w-0 flex-1">
				<div className="flex items-center gap-2">
					...
					{isModified && !isNew && <Badge variant="secondary" className="text-[10px]">modified</Badge>}
				...
				<div className="flex items-center gap-2 text-xs text-muted-foreground">
					...
					{chance !== null && (
						<>
							<span>·</span>
							<span>{Math.round(chance * 100)}/{Math.round((1 - chance) * 100)}</span>
						</>
					)}
					{rolloutFunction !== null && (
						<>
							...
							<span>·</span>
							<span>{rolloutPointCount} point{rolloutPointCount === 1 ? "" : "s"}</span>
						</>
					)}
					{updatedAtMs && (
						<>
...
export function Page() {
	...
	const currentDraft = selectedStageSlug === null
	...
	useEffect(() => {
		...
		void ensureDefaultStages({
			expectedOrgSlug: project.orgSlug,
			projectSlug: project.projectSlug,
		}).catch((e: unknown) => toast.error(e instanceof Error ? e.message : "Failed to initialize stages."));
	...
	const rows = useMemo(() => {
		const existing: Array<VariableDisplayRow> = persistedRows
			.map((r) => ({
				key: `existing-${r.id}`,
	...
	async function openEditExistingDialog(row: Extract<VariableDisplayRow, { type: "existing" }>) {
		...
		try {
			...
			updateCurrentStageDraft((current) => {
				current.revealedValues[row.id] = decrypted.kind === "secret"
					? { kind: "secret", declaredType: decrypted.declaredType, value: decrypted.value }
					: decrypted.kind === "ab_roll"
						? { kind: "ab_roll", declaredType: decrypted.declaredType, valueA: decrypted.valueA, valueB: decrypted.valueB, chance: decrypted.chance }
						: { kind: "rollout", declaredType: decrypted.declaredType, valueA: decrypted.valueA, valueB: decrypted.valueB, rolloutFunction: decrypted.rolloutFunction, rolloutMilestones: decrypted.rolloutMilestones };
	...
	async function importTextSource(source: string, text: string): Promise<void> {
		...
		applyImportedEntries(
			parsed.entries.map((e) => ({ name: e.name, value: e.value, source, lineNumber: e.lineNumber })),
	...
	async function importFiles(files: Array<File>): Promise<void> {
		...
		for (const file of files) {
			...
			const parsed = parseEnvText(text);
			allEntries.push(...parsed.entries.map((e) => ({ name: e.name, value: e.value, source: file.name, lineNumber: e.lineNumber })));
	...
	async function handleSaveAll(): Promise<void> {
		...
		try {
			...
			updates = persistedRows
				.filter((r) => !currentDraft.deletedIds[r.id] && currentDraft.updatedValues[r.id] !== undefined)
				.map((r) => {
		...
		try {
			...
			toast.success(`Saved ${result.createdCount} created, ${result.updatedCount} updated, ${result.deletedCount} deleted.`);
		} catch (e: unknown) {
	...
	return (
		<div className="space-y-4">
							...
							<div className="overflow-hidden rounded-lg border">
								{filteredRows.length === 0 ? (
									...
									filteredRows.map((row) => {
										const isDeleted = row.type === "existing" && Boolean(currentDraft.deletedIds[row.id]);
										const isModified = row.type === "existing" && currentDraft.updatedValues[row.id] !== undefined;
			...
			<VariableEditorDialog
				...
				isExistingKindLocked={dialogState.mode === "edit-existing" && dialogState.row.kind === "rollout"}
			/>
