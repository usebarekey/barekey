...
const codeExamples = [
	{
		...
		description: "Retrieve secrets with full type safety. Coerce values and set fallback defaults in a single chain.",
...
const debug = env.get("DEBUG_MODE")
  ...
  .default(false);`,
	},
	{
		...
		description: "Assign users to experiment cohorts with a stable seed. Same user, same variant, every request.",
		code: `import { env } from "@barekey/sdk";
...
if (variant === "treatment") {
  ...
  renderClassicCheckout();
	...
	},
	{
		...
		description: "Ship features gradually with dynamic evaluation and time-based cache invalidation.",
		code: `import { env } from "@barekey/sdk";
				...
				<div className="relative grid gap-2 md:min-h-[80vh] md:grid-cols-4 md:grid-rows-3">
					...
					<div className="flex flex-col justify-between overflow-hidden border bg-card p-5">
						...
						<div className="mt-4 flex flex-col gap-1">
							...
							<div className="flex items-center gap-2">
								<div className="h-1.5 w-1.5 bg-foreground/25" />
								<span className="font-mono text-[10px] text-muted-foreground/50">staging</span>
							</div>
							<div className="flex items-center gap-2">
