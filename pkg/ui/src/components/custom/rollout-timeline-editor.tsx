  ...
  for (let index = 0; index < points.length - 1; index += 1) {
    ...
    if (rolloutFunction === "ease_in_out") {
      for (let sample = 1; sample <= 12; sample += 1) {
        ...
        const percentage =
          current.percentage + (next.percentage - current.percentage) * easedProgress;
...
export function RolloutTimelineEditor({
  ...
  const xForMs = (value: number): number => {
    ...
    return (
      plotBounds.left +
      ((value - timelineDomain.startMs) / range) * (plotBounds.right - plotBounds.left)
    );
  };
  ...
  function updatePoint(
    ...
    const clampedPercentage = roundPercentage(
      clamp(nextValue.percentage ?? current.percentage, 0, 100),
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-xl border bg-gradient-to-br from-secondary/20 via-background to-background">
        ...
        <div className="p-4">
          ...
          >
            ...
            {areaPath ? (
              <path d={areaPath} fill="url(#rollout-area)" className="text-primary" />
            ) : null}
          ...
          <Input
            ...
            onChange={(event) => {
              ...
              updatePoint(selectedPointIndex, {
                atMs: Date.parse(
                  fromLocalDateTimeInputValue(event.currentTarget.value, selectedMilestone.at),
                ),
              });
            }}
      ...
      {selectedPoint ? (
        <p className="text-xs text-muted-foreground">
          Selected point:{" "}
          {format(new Date(selectedMilestone?.at ?? addHours(new Date(), 0)), "MMM d, yyyy HH:mm")}{" "}
          at {selectedPoint.percentage.toFixed(1)}% for value B.
        </p>
      ) : null}
    </div>
  );
