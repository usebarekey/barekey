 ...
 * @author GPT-5.4
 */
function toBillingStateError(error: unknown): AuthError | ExternalServiceError | ValidationError {
  if (
  ...
  handler: (args) =>
    Effect.gen(function* () {
      ...
      return yield* Effect.tryPromise({
        try: () => getBillingStateForCurrentOrgHandler(ctx, args),
        catch: toBillingStateError,
      });
    }),
});
