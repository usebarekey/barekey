...
import {
  cancelScheduledBatchExecutionEffect,
  getEditableScheduledBatchEffect,
  patchScheduledBatchEffect,
  rescheduleScheduledBatchExecutionEffect,
} from "./repo";
...
function updateForCurrentOrgProjectEffect(
  ...
  return Effect.gen(function* () {
    ...
    const existingSchedule: Doc<"projectVariableSchedules"> | null =
      yield* getEditableScheduledBatchEffect(ctx, String(args.scheduleId));
    ...
    if (existingSchedule.scheduledFunctionId !== null) {
      yield* cancelScheduledBatchExecutionEffect(
        ctx,
        String(existingSchedule.scheduledFunctionId),
      );
    }

    const scheduledFunctionId = yield* rescheduleScheduledBatchExecutionEffect(ctx, {
      runAtMs,
      scheduleId: String(existingSchedule._id),
    });

    const updatedAtMs = clock.nowMs();
    yield* patchScheduledBatchEffect(ctx, {
      scheduleId: String(existingSchedule._id),
      stageSlug: stage.slug,
      timezone,
      runAtMs,
