...
import {
  getScheduledBatchEffect,
  insertScheduledBatchEffect,
  scheduleScheduledBatchExecutionEffect,
} from "./repo";
...
function createForCurrentOrgProjectEffect(
  ...
  return Effect.gen(function* () {
    ...
    const scheduleId = yield* insertScheduledBatchEffect(ctx, {
    ...
    const scheduledFunctionId = yield* scheduleScheduledBatchExecutionEffect(ctx, {
      runAtMs,
    ...
    const currentSchedule: Doc<"projectVariableSchedules"> | null =
      persistedSchedule ?? (yield* getScheduledBatchEffect(ctx, String(scheduleId)));
