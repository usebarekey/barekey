...
import { projectVariableValidationError } from "./errors";
import type {
...
export function summarizePreparedWriteApplication(
  ...
  for (const variableId of deletedIds) {
    ...
    if (existing === undefined) {
      throw projectVariableValidationError("Variable delete target does not exist.");
    }
  ...
  for (const update of input.updates) {
    ...
    if (existing === undefined) {
      throw projectVariableValidationError("Variable update target does not exist.");
    }
    if (deletedIds.has(update.id)) {
      throw projectVariableValidationError(
